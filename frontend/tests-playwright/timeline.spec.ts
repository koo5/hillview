...
test.describe('Timeline walk', () => {
	...
	test('t opens the timeline panel, lists photos, toggles width, and closes', async ({ page, testUsers }) => {
		...
		await expect(page.locator('.bearing-circle.selected')).toBeVisible({ timeout: T(10000) });

		// 't' toggles the timeline open for the current front photo.
		await page.keyboard.press('t');

		const panel = page.getByTestId('timeline-panel');
		await expect(panel).toBeVisible({ timeout: T(15000) });
		...
		await expect(rows.first()).toBeVisible({ timeout: T(15000) });
		...
		await page.keyboard.press('t');
		await expect(panel).toBeHidden({ timeout: T(10000) });
	...
	test('jumping a row and stepping with . / , move the cursor', async ({ page, testUsers }) => {
		...
		await expect(status).toHaveText(`1 / ${n}`, { timeout: T(10000) });

		// '.' steps to the next (newer) photo → "2 / n".
		await page.keyboard.press('.');
		await expect(status).toHaveText(`2 / ${n}`, { timeout: T(10000) });
	...
	test('add-user picker adds and removes a tracked user', async ({ page, testUsers }) => {
		...
		await expect(page.getByTestId('timeline-user')).toHaveCount(1, { timeout: T(15000) });
		...
		await expect(options.first()).toBeVisible({ timeout: T(15000) });

		// Adding one → two tracked users; the picker closes.
		await options.first().click();
		await expect(page.getByTestId('timeline-user')).toHaveCount(2, { timeout: T(15000) });
	...
	test('content filters narrow the walk, and it reloads live while open', async ({ page, testUsers }) => {
		...
		await expect(status).toHaveText('No photos', { timeout: T(15000) });
