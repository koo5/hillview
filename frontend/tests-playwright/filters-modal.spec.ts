...
test.describe('Filters with uploaded photos', () => {
	...
	test('applying a filter should hide unanalyzed photos on map', async ({ page, testUsers }) => {
		...
		await expect(markersBeforeFilter).not.toHaveCount(0, { timeout: T(15000) });
		...
		await expect(markersWithFilter).not.toHaveCount(0, { timeout: T(15000) });
	});
	...
	test('disabling show-unanalyzed should hide unanalyzed photos', async ({ page, testUsers }) => {
		...
		await expect(markersBefore).not.toHaveCount(0, { timeout: T(15000) });
		...
		await expect(markers).not.toHaveCount(0, { timeout: T(15000) });
		const markerCount = await markers.count();
		const grayedCircles = page.locator('[data-testid^="photo-marker-"] .bearing-circle.grayed');
		await expect(grayedCircles).toHaveCount(markerCount, { timeout: T(10000) });
	});
	...
	test('re-enabling show-unanalyzed should bring photos back', async ({ page, testUsers }) => {
		...
		await expect(page.locator('[data-testid^="photo-marker-"]')).not.toHaveCount(0, { timeout: T(15000) });
		...
		await expect(grayedCircles).toHaveCount(0, { timeout: T(10000) });
	});
});
