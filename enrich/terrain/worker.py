...
Environment:
    TERRAIN_DEM_PATH   EPSG:4326 mosaic (COG/VRT) — see README.md for the
                       ČÚZK DMP 1G / Copernicus GLO-30 pipeline that builds it
...
DEM_PATH = os.getenv("TERRAIN_DEM_PATH", "")
...
# dropped so a stray payload can't reach exotic renderer internals.
...
def _render(lat: float, lon: float, params: dict):
    import renderer
    if not DEM_PATH:
        raise RuntimeError("TERRAIN_DEM_PATH not set (see enrich/terrain/README.md)")
    kwargs = {k: v for k, v in (params or {}).items() if k in RENDER_KEYS}
    max_d = float(kwargs.get("max_distance_m", 100_000.0))
    dem = renderer.load_geotiff_window(DEM_PATH, lat, lon, max_d * 1.05)
