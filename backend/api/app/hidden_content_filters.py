...
def apply_hidden_content_filters(
	...
	#log.debug(f"apply_hidden_content_filters called: user_id={current_user_id}, photo_source={photo_source}")

	if not current_user_id:
		# No filtering for anonymous users
		log.debug("No current_user_id, skipping filtering")
		return query

	# Filter out photos explicitly hidden by the user
	#log.debug(f"Applying photo filtering for user {current_user_id}, source {photo_source}")
	...
	#log.debug(f"Hidden photo subquery: {hidden_photo_subquery}")

	query = query.where(
		Photo.id.notin_(hidden_photo_subquery)
	)
	#log.debug(f"Applied photo filtering, modified query: {query}")
...
def apply_mapillary_hidden_content_filters(
	...
	#log.debug(f"apply_mapillary_hidden_content_filters called with user_id: {current_user_id}")

	if not current_user_id:
		#log.debug("No user_id provided, skipping SQL-based filtering")
		return ""
	...
	#log.debug(f"Generated SQL filters for user {current_user_id}: {filters}")
	return filters
...
async def filter_mapillary_photos_list(
	...
	#log.debug(f"filter_mapillary_photos_list called with {len(photos)} photos, user_id: {current_user_id}")

	if not current_user_id or not photos:
		#log.debug(f"Skipping filtering: current_user_id={current_user_id}, photos_count={len(photos) if photos else 0}")
		return photos
	...
	#log.debug(f"Querying hidden photos for user {current_user_id} with {len(photo_ids)} photo IDs")
	...
	if creator_ids:
		...
		#log.debug(f"Querying hidden users for user {current_user_id} with {len(creator_ids)} creator IDs")
	...
	#log.debug(f"Filtering complete: {len(filtered_photos)} photos remaining, {filtered_count} filtered out")
