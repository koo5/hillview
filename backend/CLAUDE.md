...
```bash
# Initialize Alembic (first time only, from backend directory)
docker exec -it hillview-api alembic init alembic

# Create a new migration after model changes
docker exec -it hillview-api alembic revision --autogenerate -m "Description of changes"

# Apply migrations to database
docker exec -it hillview-api alembic upgrade head

# Downgrade to previous migration
docker exec -it hillview-api alembic downgrade -1

# View migration history
docker exec -it hillview-api alembic history

# View current migration version
docker exec -it hillview-api alembic current

# Upgrade to specific revision
docker exec -it hillview-api alembic upgrade <revision_id>

# Generate SQL only (don't apply)
docker exec -it hillview-api alembic upgrade head --sql

# Alternative: Enter container for multiple commands
docker exec -it hillview-api bash
# Then run alembic commands directly: alembic upgrade head
```
...
- **Check for data migration needs** when changing column types
...
# 2. Make changes to models in app/models.py

# 3. Generate migration from container
docker exec -it hillview-api alembic revision --autogenerate -m "Add new user field"

# 4. Review the generated migration file (appears in backend/alembic/versions/)

# 5. Apply migration
docker exec -it hillview-api alembic upgrade head

# 6. Verify database changes
docker exec -it hillview-db psql -U postgres -d hillview -c "\d users"
```

#### Container-Specific Notes
- **Container Name**: Use `hillview-api` (check with `docker ps` if different)
- **Migration Files**: Generated in host filesystem at `backend/alembic/versions/`
- **Database Access**: Connect via `hillview-db` container for verification
- **Environment Variables**: Database URL automatically configured in container
