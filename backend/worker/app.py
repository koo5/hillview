...
class BrowserMetadata(BaseModel):
	...
	accuracy: Optional[float] = None
	# The Android fast-write path's stamp. That path does not write EXIF into
	# the file at all (the whole-file rewrite is the throughput cost it exists
	# to avoid), so these reach the worker ONLY here, and photo_processor folds
	# them into the synthesized UserComment.
	#
	# They must be DECLARED to arrive: this model is the door, and pydantic
	# drops undeclared keys silently — the client sent location_age_ms and
	# exposure for weeks while the worker never saw them, and nothing anywhere
