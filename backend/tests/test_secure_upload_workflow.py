...
class TestSecureUploadWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_phase2_upload_authorization(self, test_user_auth):
    ...
    @pytest.mark.asyncio
    async def test_phase3_worker_upload_processing(self, test_image, test_user_auth):
        ...
        upload_token = auth_data["upload_token"]
        worker_url = auth_data["worker_url"]
        photo_id = auth_data["photo_id"]
        
        # Generate a real client signature (in practice, client would sign the upload data)
        client_signature = "real_ecdsa_signature_from_client_private_key"
    ...
    async def test_complete_secure_upload_workflow(self, test_image):
        ...
        auth_data = None
        try:
            auth_data = await self.test_phase2_upload_authorization(auth_token)
            phase2_success = auth_data is not None
        except Exception as e:
            print(f"❌ Phase 2 failed: {e}")
            phase2_success = False
        
        # Phase 3: Worker Processing (only if Phase 2 succeeded)
        print(f"\n--- Phase 3: Worker Processing ---")
        phase3_success = False
        if phase2_success and auth_data:
            try:
                upload_token = auth_data["upload_token"]
                worker_url = auth_data["worker_url"]
                ...
                async with httpx.AsyncClient() as client:
                    with open(test_image, 'rb') as f:
                        files = {'file': ('workflow_test.jpg', f, 'image/jpeg')}
                        data = {'client_signature': 'test_client_signature'}
                        headers = {'Authorization': f'Bearer {upload_token}'}
            ...
            print(f"⏭️ Phase 3: Skipped (Phase 2 failed)")
        
        # Phase 4: Final Verification - Client checks photo was processed
        print(f"\n--- Phase 4: Final Verification ---")
        phase4_success = False
        if phase3_success and auth_data:
            try:
                photo_id = auth_data["photo_id"]
                async with httpx.AsyncClient() as client:
            ...
            print(f"⏭️ Phase 4: Skipped (Phase 3 failed)")
