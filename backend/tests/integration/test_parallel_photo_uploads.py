...
class TestParallelPhotoUploads(BasePhotoTest):
    ...
    async def upload_single_photo(self, photo_id: int) -> Dict:
        ...
        try:
            ...
            photo_uuid = await upload_test_image(
                filename, image_data, description, self.test_token, is_public=True
            )
            ...
            final_status = wait_for_photo_processing(photo_uuid, self.test_token, timeout=60)
            result['processing_time'] = time.time() - processing_start

            if final_status and final_status.get('processing_status') == 'completed':
                result['success'] = True
                print(f"✓ Photo {photo_id} uploaded successfully in {result['upload_time']:.2f}s, processed in {result['processing_time']:.2f}s")
            else:
                result['error'] = f"Processing failed: {final_status}"
                print(f"✗ Photo {photo_id} processing failed: {final_status}")

        except Exception as e:
            result['error'] = str(e)
            result['upload_time'] = time.time() - start_time
            print(f"✗ Photo {photo_id} upload failed: {e}")
    ...
    async def test_parallel_upload_medium_batch(self):
        ...
        if successful_uploads:
            ...
            processing_times = [r['processing_time'] for r in successful_uploads]

            print(f"Upload time - avg: {sum(upload_times)/len(upload_times):.2f}s, "
                  f"min: {min(upload_times):.2f}s, max: {max(upload_times):.2f}s")
            print(f"Processing time - avg: {sum(processing_times)/len(processing_times):.2f}s, "
                  f"min: {min(processing_times):.2f}s, max: {max(processing_times):.2f}s")
    ...
    async def test_parallel_upload_large_batch(self):
        ...
        if successful_uploads:
            ...
            print(f"Upload times - avg: {sum(upload_times)/len(upload_times):.2f}s, "
                  f"min: {min(upload_times):.2f}s, max: {max(upload_times):.2f}s")
            print(f"Processing times - avg: {sum(processing_times)/len(processing_times):.2f}s, "
                  f"min: {min(processing_times):.2f}s, max: {max(processing_times):.2f}s")
